...
python __anonymous () {
    ...
    elif kerneltype == 'Image.gz':
        # Override KERNEL_IMAGETYPE_FOR_MAKE variable, which is internal
        # to kernel.bbclass . We override the variable here, since
        # kernel.bbclass strips the .gz extension from it, but the kernel
        # aarch64 build system does include an Image.gz target
        d.setVar("KERNEL_IMAGETYPE_FOR_MAKE", "Image.gz")
}

do_uboot_mkimage() {
    if test "x${KERNEL_IMAGETYPE}" = "xuImage" ; then
        uboot-mkimage -A ${UBOOT_ARCH} -O linux -T kernel -C gzip -a ${UBOOT_LOADADDRESS} -e ${UBOOT_ENTRYPOINT} -n "${DISTRO_NAME}/${PV}/${MACHINE}" -d arch/${ARCH}/boot/${KERNEL_IMAGETYPE_FOR_MAKE} arch/${ARCH}/boot/uImage
    fi
}
