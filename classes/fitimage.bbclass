...
FITIMAGE_DTB_LOADADDRESS ??= "0x9000"
...
def fitimage_emit_fit_header(d,fd):
     import shutil
     fd.write('/dts-v1/;\n\n/ {\n')
     fd.write(d.expand('\tdescription = "U-boot fitImage for ${DISTRO_NAME}/${PV}/${MACHINE}";\n'))
...
def fitimage_emit_section_dtb(d,fd,dtb_file,dtb_path):
    ...
    arch = d.getVar("TARGET_ARCH", True)
    DTB_LOADADDRESS = d.expand("${FITIMAGE_DTB_LOADADDRESS}")
    arch = "arm64" if arch == "aarch64" else arch
    fd.write('\t\t'     + 'fdt@%s {\n' % dtb_file)
...
do_deploy() {
    ...
    linux_bin_symlink_name="${MACHINE}"
    printf 'Copying fitImage file...'
    install -m 0644 ${B}/fitImage ${DEPLOYDIR}/${linux_bin_base_name}.itb

    cd ${DEPLOYDIR}
    ln -sf ${its_base_name}.its ${its_symlink_name}.its
    ln -sf ${linux_bin_base_name}.itb ${linux_bin_symlink_name}.itb
}
